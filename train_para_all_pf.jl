...
function my_get_data(file_path; n = 5000, Δsamples = 1, grid_size = div(1001, Δsamples), T = Float32)
...
function my_get_dataloader(; ratio::Float64 = 0.9, batchsize = 128,dense=false)
    if dense
        𝐱1, 𝐲1, safe1, pf1 = my_get_data("data_bcks_parabolic_abs_test_dense.mat") # _new_10
    
        data_train1, data_test1 = splitobs((𝐱1, 𝐲1, safe1, pf1), at = ratio)
        𝐱2, 𝐲2, safe2, pf2 = my_get_data("data_ppo_parabolic_abs_test_dense.mat")
        
        data_train2, data_test2 = splitobs((𝐱2, 𝐲2, safe2, pf2), at = ratio)
        𝐱3, 𝐲3, safe3, pf3 = my_get_data("data_sac_parabolic_abs_test_dense.mat")
        
        data_train3, data_test3 = splitobs((𝐱3, 𝐲3, safe3, pf3), at = ratio)
    else
        𝐱1, 𝐲1, safe1, pf1 = my_get_data("data_bcks_parabolic_abs.mat") # _new_10
        
        data_train1, data_test1 = splitobs((𝐱1, 𝐲1, safe1, pf1), at = ratio)
        𝐱2, 𝐲2, safe2, pf2 = my_get_data("data_ppo_parabolic_abs.mat")
        
        data_train2, data_test2 = splitobs((𝐱2, 𝐲2, safe2, pf2), at = ratio)
        𝐱3, 𝐲3, safe3, pf3 = my_get_data("data_sac_parabolic_abs.mat")
...
function train(; cuda = true, η₀ = 1.0f-3, λ = 1.0f-4, epochs = 500)
    ...
    model = FourierNeuralOperator(ch = (2, 64, 64, 64, 64, 64, 128, 1), modes = (16,), 
                                  σ = gelu)
    data = my_get_dataloader(;dense=false)
    ...
    @save "model/para_FNO_all.bson" model

    return learner
end

function train_dense(; cuda = true, η₀ = 1.0f-3, λ = 1.0f-4, epochs = 500)
    if cuda && CUDA.has_cuda()
        device = gpu
        CUDA.allowscalar(false)
        @info "Training on GPU"
    ...
    model = FourierNeuralOperator(ch = (2, 64, 64, 64, 64, 64, 128, 1), modes = (16,), 
                                  σ = gelu)
    data = my_get_dataloader(;dense=true)
    optimiser = Flux.Optimiser(Flux.Optimise.WeightDecay(λ), Flux.Adam(η₀))
    loss_func = l₂loss
    ...
    fit!(learner, epochs)
    model = learner.model |> cpu
    @save "model/para_FNO_all_dense.bson" model
...
function train_MNO_dense(; cuda = true, η₀ = 1.0f-3, λ = 1.0f-4, epochs = 500)
    ...
    fit!(learner, epochs)
    model = learner.model |> cpu
    @save "model/para_MNO_all_dense.bson" model

    return learner
end

train(epochs=100) 
# train_dense(epochs=100) 
